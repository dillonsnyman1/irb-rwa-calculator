...
`infra/bootstrap` is self-contained and maintains its own Terraform state.
It needs to be applied once for this repo, producing its own state bucket,
lock table and OIDC deploy role.
...
4. The CloudFront URL is printed in the workflow's job summary
   ...
   terraform init -backend-config="bucket=<state_bucket_name>" \
     -backend-config="key=irb-rwa-calculator/terraform.tfstate" \
